...
let API_KEY : String = GlobalConfig.speechRecognitionKey
let HOST : String = GlobalConfig.speechRecognitionHost

typealias SpeechRecognitionCompletionHandler = (StreamingRecognizeResponse?, NSError?) -> (Void)

struct SpeechRecognitionServiceResponse {
    var transcript: String
    var accuracy: Double
    
    init(transcript: String, accuracy: Double) {
        self.transcript = transcript
        self.accuracy = accuracy
    }
    
    //TODO implement initalizer for SpeechRecognitionCompletionHandler
}
...
class SpeechRecognitionService {
    ...
    func streamAudioData(_ audioData: NSData, completion: @escaping SpeechRecognitionCompletionHandler) {
        if (!streaming) {
            ...
            recognitionConfig.languageCode = GlobalConfig.speechRecognitionLanguage
            recognitionConfig.maxAlternatives = 5
            recognitionConfig.enableWordTimeOffsets = true
            
            // add custom phrase hints to detect brands
            if let _ = self.phraseHints {
                let speechContext: SpeechContext = SpeechContext()
                speechContext.phrasesArray = NSMutableArray(array: self.phraseHints!)
