...
class MarymoodSpider(Spider):
    name = 'Mary Mood'
    allowed_domains = ['marymood.com']

    def __init__(self, *a, **kw):
        super(MarymoodSpider, self).__init__(*a, **kw)
        self.urls = [{"value": "https://marymood.com/en/12-shop", "gender": ["women"]}]
    ...
    def get_name(self, response):
        name = response.xpath('//h1[@class="breadcrumb-product"]/text()').extract_first()
        return name

    def get_image_urls(self, response):
        urls = response.xpath('//ul[@id="thumbs_list_frame"]//a/@href').extract()
        return urls

    def get_description(self, response):
        description = response.xpath('//div[@id="short_description_block"]').extract()
        if not description:
            return
