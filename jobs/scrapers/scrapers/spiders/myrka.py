...
class MyrkaSpider(Spider):
    name = 'Myrka'
    allowed_domains = []
    currency_symbol_map = {'£': 'GBP', '$': 'USD', '€': 'EUR'}

    def __init__(self, *a, **kw):
        super(MyrkaSpider, self).__init__(*a, **kw)
        self._pagination_pattern = re.compile('^https?:.*[\\?&]page=\\d{1}$')
        self.urls = [{"value": "http://myrkastudios.com/product-category/women/", "gender": ["women"]}, {"value": "http://myrkastudios.com/product-category/men/", "gender": ["men"]}]
