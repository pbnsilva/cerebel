...
var g_baseFolderPath = null;
var g_imgFolderPath = null;
var g_annoFolderPath = null;
var g_curListItem = null;
var g_curDIV = null;
var g_marksXML = null;
var g_categoriesArr = null;
var g_listItemCursor = 0;
var g_bndBoxCnt = 0;
var g_zoom = 100;
...
const CATEGORIES_TXT = 'category.txt';
const MAX_FILES_LOAD = 2000;

$(document).ready(function () {

  /**
   * Read categories from file category list.
   */
  var fs = require('fs');
  var text = fs.readFileSync(CATEGORIES_TXT).toString('utf-8');
  g_categoriesArr = text.split("\n");
  g_categoriesArr.splice(-1);  // remove last new line
  generateCategoriesList(g_categoriesArr);
  ...
  if (win) {
    ...
    win.on('close', function () {
      if (g_categoriesArr) {
        var fs = require('fs');
        fs.writeFile(__dirname + '/' + CATEGORIES_TXT, g_categoriesArr.join('\n'));
      }
    });
    win.maximize();
  }
  ...
  $('#btn_reload').click(function () {
    if (g_baseFolderPath == null) return;
    if ($('#modal_del_confirm').hasClass('in'))
      $('#modal_del_confirm').modal('hide');
    loadFiles(g_baseFolderPath);
  });
  ...
  $('#btn_del_file').click(function () {
    if (!g_curListItem || !g_baseFolderPath) return;
    var fs = require('fs');
    var curItemName = g_curListItem.html().toString();
    var picPath = g_imgFolderPath.toString() + '/' + curItemName;
    fs.unlink(picPath, function (err) {
      ...
      if(g_curListItem.hasClass('list-group-item-success')){
        var xmlPath = g_annoFolderPath.toString() + '/' + curItemName.substring(0, curItemName.lastIndexOf('.')) + '.xml';
  ...
  $('#input_class').change(function () {
    ...
    var flag = false;
    for (var i = 0, l = g_categoriesArr.length; i < l; i++) {
      if (g_categoriesArr[i] == $(g_curDIV).attr('data-class-name')) {
        $(g_curDIV).attr('data-sel', g_categoriesArr[i]);
        flag = true;
        break;
      }
    }
    if (!flag) {
      var value = $(g_curDIV).attr('data-class-name');
      g_categoriesArr.push(value);
      $(g_curDIV).attr('data-sel', value);
    }
    for (var i = 0, l = g_marksXML.object.length; i < l; i++) {
      if (g_marksXML.object[i].id == g_curDIV.id) {
  ...
  $('#div_container').on("click", '.img-bndbox', function (e) {
    ...
    _timer = setTimeout(function () {
      ...
      if (!$(g_curDIV).attr('data-class-name')) {
        var flag = false;
        for (var i = 0, l = g_categoriesArr.length; i < l; i++) {
          if (g_categoriesArr[i] == $(g_curDIV).attr('data-sel')) {
            $(g_curDIV).attr('data-class-name', g_categoriesArr[i]);
            flag = true;
            break;
          }
        }
        if (!flag) {
          g_categoriesArr.push($(g_curDIV).attr('data-sel'));
  ...
  $(window).mouseup(function (e) {
    ...
    $('#input_class').val(g_categoriesArr[0]);
    $('#input_class').attr('value', g_categoriesArr[0]);
...
/**
 * Utility functions
 * dir is the base dir and expects standard Pascal VOC directory structure:
 * dir/JPEGImages
 * dir/Annotations
 */
function loadFiles(dir) {
  if (dir == null) return;
  if (dir != g_baseFolderPath) g_listItemCursor = 0;
  g_baseFolderPath = dir;
  g_imgFolderPath = dir + "/JPEGImages";
  g_annoFolderPath = dir + "/Annotations";
  var fs = require('fs');
  fs.readdir(g_imgFolderPath.toString(), function (err, files) {
    if (g_listItemCursor == 0)
      g_fileList = [];
    var curMax = g_listItemCursor + Math.min(files.length, MAX_FILES_LOAD);
    for (var i = g_listItemCursor, l = curMax; i < l; i++) {
      if ($.inArray(files[i].substring(files[i].lastIndexOf('.') + 1, files[i].length).toLowerCase(), IMG_EXTENSIONS) >= 0) {
        var xmlPath = g_annoFolderPath + '/' + files[i].substring(0, files[i].lastIndexOf('.')) + '.xml';
        try {
...
function generateFilesList(fileList, filter) {
  ...
  if(c == 0) {
    $('#div_files_list').html('<li class="list-group-item list-group-item-danger">No Image Found!</li>');
      ...
      var node = '<a href="#" id="loadMore" class="list-group-item">Load more...</a>';
      $('#div_files_list').append(node);
  }
...
function saveToXML(file) {
  ...
  var xmlURL = g_annoFolderPath.toString() + '/' + file.substring(0, file.lastIndexOf('.')) + '.xml';
...
function updateMarks() {
  if (g_curListItem) {
    if ($('#div_container .img-bndbox').length != 0) {
      ...
      var xmlPath = g_annoFolderPath.toString() + '/' + g_curListItem.html().substring(0, g_curListItem.html().lastIndexOf('.')) + '.xml';
      var fs = require('fs');
      fs.unlink(xmlPath, function (err) {
        //if (err) throw err;
        if (!err)
...
function generateCategoriesList(arr) {
  $('#ul_classes_list').html('');
  for (var i = 0; i < arr.length; i++) {
    $('#ul_classes_list').append('<li><a href="javascript:void(0)" index="' + i + '" value="' + arr[i] + '">'
      + arr[i] + '</a></li>');
  }
}
